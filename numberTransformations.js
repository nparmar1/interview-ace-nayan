...
const getNeighbors = (num, additiveNums, multiplicativeNums) => {
    ...
    for (const additiveNum of additiveNums) {
        const neighbor = num + additiveNum;

        neighbors.push(neighbor);
    }

    for (const multiplicativeNum of multiplicativeNums) {
        const neighbor = num * multiplicativeNum;
...
const findShortestTransformationLength = (
    ...
    queue.enqueue({ num: startNum, numOfOperationsSoFar: 0 });

    while (queue.size() > 0) {
        // Remove node
        const { num, numOfOperationsSoFar } = queue.dequeue();

        // Process node
        if (num === targetNum) return numOfOperationsSoFar;
        ...
        for (const neighbor of neighbors) {
            ...
            queue.enqueue({ num: neighbor, numOfOperationsSoFar: numOfOperationsSoFar + 1 });
        }
    }
};
